...
module.exports = function(secret, options = {}) {
    ...
    return function(request, response, next) {
        ...
        if (!(options.header in request.headers)) {
            return handleError(`Header provided not in sent headers. Expected ${options.header} but not found in request.headers`);
        }
        ...
        if (!request.headers[options.header].startsWith(options.identifier)) {
            return handleError(`Header did not start with correct identifier. Expected ${options.identifier} but not found in options.header`);
        }
        ...
        if (!unixMatch || unixMatch.length !== 2) {
            return handleError('Unix timestamp was not present in header');
        }
        ...
        if (timeDiff > options.maxInterval || timeDiff < 0) {
            return handleError('Time difference between generated and requested time is too great');
        }
        ...
        if (hashMatch.length !== 2) {
            return handleError('HMAC digest was not present in header');
        }
        ...
        if (hmacDigest.length !== sourceDigest.length || !crypto.timingSafeEqual(hmacDigest, sourceDigest)) {
            return handleError('HMAC\'s did not match');
        }
